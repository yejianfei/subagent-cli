...
async function sendPrompt(sessionId, text, timeoutMs = 120_000) {
  try {
    ...
    if (state === 'ASKING') return { success: true, data: { status: 'approval_needed' } }
    if (state === 'IDLE') return { success: true, data: { status: 'done' } }
...
/** Clean Gemini CLI session data for a given CWD */
function cleanGeminiSessions(cwd) {
  const projectName = basename(cwd)
  const geminiHome = join(homedir(), '.gemini')
  const paths = [
    join(geminiHome, 'tmp', projectName),
    join(geminiHome, 'history', projectName),
  ]
...
describe('Gemini Preflight', () => {
  it('P1: gemini CLI installed', () => {
    try {
      const version = execSync('gemini --version 2>&1', { encoding: 'utf-8', timeout: 10_000 }).trim()
      console.log(`    Gemini CLI: ${version}`)
    } catch {
      assert.fail('Gemini CLI not found. Install via: npm i -g @anthropic-ai/gemini-cli or brew install gemini-cli')
    }
  })
  ...
  it('P3: clean stale sessions', () => {
    ...
    // Clean leftover /tmp/subagent-gemini-* from previous interrupted runs
    try {
      const tmpFiles = readdirSync(tmpdir())
      let count = 0
      for (const f of tmpFiles) {
        if (f.startsWith('subagent-gemini-')) {
          cleanGeminiSessions(join(tmpdir(), f))
          rmSync(join(tmpdir(), f), { recursive: true, force: true })
          count++
        }
      }
      if (count) console.log(`    Cleaned ${count} stale /tmp dir(s) + Gemini CLI sessions`)
    } catch { /* ignore */ }
  })
  ...
  it('P4: warm-start daemon', async () => {
    ...
    const gemini = json.data.subagents.find(s => s.name === 'gemini')
    assert.ok(gemini, 'gemini subagent not found in config')
    assert.equal(gemini.adapter, 'gemini-cli')
    console.log(`    Daemon ready, gemini adapter: ${gemini.adapter}`)
  })
})

// ══════════════════════════════════════════════════════════════════
// Gemini E2E: Single session full workflow
// ══════════════════════════════════════════════════════════════════

describe('E2E: Gemini single session', { timeout: 900_000 }, () => {
  let sessionId
  const tmpDir = mkdtempSync(join('/tmp', 'subagent-gemini-e2e-'))

  // ── Open + Basic ──

  it('① open gemini session', async () => {
    const { json } = await cli(['open', '-s', 'gemini', '--cwd', tmpDir], 300_000)
  ...
  it('② status → IDLE', async () => {
    ...
    assert.equal(json.data.subagent, 'gemini')
  })

  // ── Create file + approve ──

  it('③ prompt: create JS file', async () => {
    await assertCheck(sessionId, 'IDLE')
  ...
  it('⑬ approve with text: ignored for gemini (no amend)', async () => {
    await assertCheck(sessionId, 'IDLE')
    const json = await sendPrompt(sessionId,
  ...
  it('⑮½ output last → extracted reply without TUI chrome', async () => {
    ...
    assert.ok(!last.data.content.includes('Type your message'), 'Should not contain input placeholder')
  ...
  it('⑱ resume from disk', async () => {
    ...
    const { json } = await cli(['open', '--session', sessionId], 300_000)
  ...
  it('㉑ exit: graceful process exit from IDLE', async () => {
    ...
    console.log('    Exit: Ctrl+D×2 processed, state=CLOSED')
  })
  ...
  after(async () => {
    ...
    cleanGeminiSessions(tmpDir)
    rmSync(tmpDir, { recursive: true, force: true })
  })
})

// ══════════════════════════════════════════════════════════════════
// New features: check --wait, sessions --status, delete --closed, --role
// ══════════════════════════════════════════════════════════════════

describe('E2E: Gemini new features', { timeout: 600_000 }, () => {
  let sessionId
  let tmpDir

  it('㉓ warm-start daemon + open gemini session', async () => {
    tmpDir = mkdtempSync(join(tmpdir(), 'subagent-gemini-new-'))
  ...
  it('㉚ open --role overrides config role', async () => {
    const customRole = 'You are a senior Java architect.'
    const roleDir = mkdtempSync(join(tmpdir(), 'subagent-gemini-role-'))
    const { json } = await cli(['open', '-s', 'gemini', '--cwd', roleDir, '--role', customRole], 300_000)
